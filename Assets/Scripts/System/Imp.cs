...
public static class Imp
{
    ...
    public static T Random<T>(this List<T> l,bool remove=false) where T:class
    {
        if (l.Count == 0)
            return null;
        T chosen = l[UnityEngine.Random.Range(0, l.Count)];
        if (remove) l.Remove(chosen);
        return chosen;
    }
    public static T RandomE<T>(this List<T> l,bool remove=false) where T:System.Enum
    {
        if (l.Count == 0)
            return (T)(object)0;
        T chosen = l[UnityEngine.Random.Range(0, l.Count)];
    ...
    public static List<T> ShuffleE<T>(this List<T> l) where T:System.Enum
    {
        if (l.Count == 0)
            return l;
        List<T> temp = new List<T>();
